...
class EvaluationResult:
    store_id: uuid.UUID
    tasks: tuple[CanonicalReplenishmentTask, ...]
...
def evaluate_replenishment(
    ...
    *,
    ...
    active_statuses = {
        ...
        CanonicalTaskStatus.CLAIMED,
        CanonicalTaskStatus.IN_PROGRESS,
    }
    active_tasks = {
        task.sku_id: task
        for task in db.scalars(
            select(CanonicalReplenishmentTask).where(
                CanonicalReplenishmentTask.tenant_id == principal.tenant_id,
                CanonicalReplenishmentTask.store_id == store.id,
                CanonicalReplenishmentTask.status.in_(active_statuses),
            )
        ).all()
    }
    rules_by_id = {rule.id: rule for rule in rules}
    created: list[CanonicalReplenishmentTask] = []
    ...
    for snapshot in snapshots.values():
        ...
        task = CanonicalReplenishmentTask(
            tenant_id=principal.tenant_id,
            store_id=store.id,
...
def patch_replenishment_task(
    ...
    task = db.scalar(
        select(CanonicalReplenishmentTask).where(
            CanonicalReplenishmentTask.tenant_id == principal.tenant_id,
            CanonicalReplenishmentTask.id == task_id,
        )
    )
    if task is None:
    ...
    allowed: dict[CanonicalTaskStatus, frozenset[CanonicalTaskStatus]] = {
        CanonicalTaskStatus.OPEN: frozenset({CanonicalTaskStatus.CLAIMED}),
        CanonicalTaskStatus.CLAIMED: frozenset({CanonicalTaskStatus.IN_PROGRESS}),
        CanonicalTaskStatus.IN_PROGRESS: frozenset({CanonicalTaskStatus.COMPLETED}),
        CanonicalTaskStatus.COMPLETED: frozenset(),
        CanonicalTaskStatus.CANCELED: frozenset(),
        CanonicalTaskStatus.EXPIRED: frozenset(),
    }
    if request.status in {CanonicalTaskStatus.CANCELED, CanonicalTaskStatus.EXPIRED}:
        if (
            task.status
            not in {
                CanonicalTaskStatus.OPEN,
    ...
    if (
        task.status in {CanonicalTaskStatus.CLAIMED, CanonicalTaskStatus.IN_PROGRESS}
    ...
    if request.status == CanonicalTaskStatus.CLAIMED:
        values.update(claimed_by_user_id=principal.user_id, claimed_at=now)
    elif request.status == CanonicalTaskStatus.IN_PROGRESS:
        values["started_at"] = now
    elif request.status == CanonicalTaskStatus.COMPLETED:
        values["completed_at"] = now
    elif request.status == CanonicalTaskStatus.EXPIRED:
    ...
    result = cast(
        CursorResult[Any],
        db.execute(
            update(CanonicalReplenishmentTask)
            .where(
                CanonicalReplenishmentTask.id == task.id,
                CanonicalReplenishmentTask.tenant_id == principal.tenant_id,
                CanonicalReplenishmentTask.version == request.version,
            )
            .values(**values)
        ),
    )
