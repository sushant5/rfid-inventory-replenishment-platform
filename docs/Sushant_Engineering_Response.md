...
release_note: Run final release verification, deploy the tagged commit, and add the hosted URL before recruiter submission.
---
...
The submitted scope uses Python 3.12, FastAPI, SQLAlchemy 2, Alembic, PostgreSQL 17, JWT authentication, Pytest, Docker Compose, and generated OpenAPI/Swagger. It has three deployable processes sharing one codebase and image: the API, a catalog worker, and an RFID/inventory event worker.
...
`render.yaml` defines a free-demo web service whose supervised launcher runs the API and both worker entry points. Compose keeps them as separate processes. It intentionally expects an externally provisioned managed PostgreSQL database rather than coupling the code to one database vendor. Deployment needs:
...
4. a reviewer admin password supplied through the host secret store; and
5. repository access for the hosting provider and reviewer.

The application role must exist before the first migration so Alembic can grant table, sequence, function, and RLS access. A free Render web instance sleeps when idle, including its workers; the launcher also retains the owner URL because pre-deploy commands are paid-only. Those are explicit demo compromises. A production deployment uses separate always-on workers and keeps the owner credential out of the runtime container. Automatic deployment should remain off after the final tagged commit. The final submission should include the base URL, `/docs`, `/openapi.json`, `/health/ready`, `/version`, and reviewer credentials sent outside the repository.
...
The repository includes PostgreSQL migrations, forced RLS policies, a non-superuser runtime role, unit and integration tests, seed data, sample CSV/EPC data, an RFID simulator, a smoke test, a full demo script, Docker Compose, a container build, CI checks, Swagger/OpenAPI, and an architecture diagram.

Release gates are Ruff, mypy, the full PostgreSQL test suite, Alembic upgrade/drift, the image build, Compose validation, and the end-to-end demo. Add the hosted URL and release tag only after the deployed commit passes them.
