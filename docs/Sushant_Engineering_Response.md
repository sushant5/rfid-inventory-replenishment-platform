...
release_note: Release verification is complete. Grant reviewer repository access and complete the hosted Render deployment before recruiter submission.
---
...
:::callout
Release candidate repository: `https://github.com/sushant5/greyorange-abacus-engineering-take-home` (private). The locally tested release is tagged `submission-code-v4`; granting reviewer access and publishing the hosted API URL remain account-level release steps before this document is sent to the recruiter.
:::
...
:::callout
Repository: `https://github.com/sushant5/greyorange-abacus-engineering-take-home` at release tag `submission-code-v4`. Before submission, verify reviewer access and add the hosted base URL, Swagger/OpenAPI/version URLs, and dedicated reviewer credentials. Keep the platform integration key in the private recruiter email, never in the repository.
:::
...
- The private source release still requires reviewer access; no hosted URL is claimed before a verified deployment exists.
...
The verified code release is tagged `submission-code-v4`. Before recruiter submission,
add the hosted URL and matching `/version` value after rerunning the same verifier
